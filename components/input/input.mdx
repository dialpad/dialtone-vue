import { Canvas, Story, Subtitle, ArgsTable } from '@storybook/addon-docs/blocks';
import DtInput from './input';

# Inputs

<Subtitle>
  Input fields are text areas used to gather information from users.
  It can have a range of options and supports several text formats including numbers.
</Subtitle>

## Base Style

Inputs are normally paired with a label, but there are times when they can be used without a label.
Placeholder text should primarily be used as a content prompt and only provided when needed.

<Canvas>
  <Story id="forms-input--default" />
</Canvas>

## Props

<ArgsTable of={DtInput} />

## Usage

```js
import { DtInput } from '@dialpad/handset';
```

```html
<dt-input
  v-model="text"
  placeholder="placeholder"
/>
```

## With Description

Provides additional information to the user regarding the input element.

<Canvas>
  <Story id="forms-input--with-description" />
</Canvas>

### Usage

```js
import { DtInput } from '@dialpad/handset';
```

```jsx
<dt-input
  v-model="text"
  placeholder="placeholder"
  description="Description"
/>
```

## With Left Icon

Provides left icon within the form input

<Canvas>
  <Story id="forms-input--with-left-icon" />
</Canvas>

### Usage

```js
import IconSend from '@dialpad/dialtone/lib/dist/vue/icons/IconSend.vue';
import { DtInput } from '@dialpad/dialtone-vue';
```

```html
<dt-input
  v-model="text"
  placeholder="placeholder"
>
  <template #leftIcon>
    <icon-send />
  </template>
</dt-input>
```

## With Right Icon

Provides right icon within the form input

<Canvas>
  <Story id="forms-input--with-right-icon" />
</Canvas>

### Usage

```js
import IconLock from '@dialpad/dialtone/lib/dist/vue/icons/IconLock.vue';
import { DtInput } from '@dialpad/dialtone-vue';
```

```html
<dt-input
  v-model="text"
  placeholder="placeholder"
>
  <template #rightIcon>
    <icon-lock />
  </template>
</dt-input>
```

## With Both Icons

Provides both icons within the form input

<Canvas>
  <Story id="forms-input--with-both-icons" />
</Canvas>

### Usage

```js
import IconSend from '@dialpad/dialtone/lib/dist/vue/icons/IconSend.vue';
import IconLock from '@dialpad/dialtone/lib/dist/vue/icons/IconLock.vue';
import { DtInput } from '@dialpad/dialtone-vue';
```

```html
<dt-input
  v-model="text"
  placeholder="placeholder"
>
  <template #leftIcon>
    <icon-send />
  </template>
  <template #rightIcon>
    <icon-lock />
  </template>
</dt-input>
```

## Validation States

Provides feedback to the user based on their interction, or lack thereof, with an input.

### Warning

Warns the user that the value(s) they've entered has a potential problem,
but it doesn't block them from proceeding with input submission.

<Canvas>
  <Story id="forms-input--with-warn" />
</Canvas>

#### Usage

```js
import { DtInput, VALIDATION_MESSAGE_TYPES } from '@dialpad/handset';
```

```html
<dt-input
  v-model="text"
  placeholder="placeholder"
  :messages="[{
    message: 'This is a warning message',
    type: VALIDATION_MESSAGE_TYPES.WARNING,
  }]"
/>
```

### Error

Alerts the user that the value(s) they've entered is incorrect,
not provided, or has some issue that will block them from proceeding with input submission.

<Canvas>
  <Story id="forms-input--with-error" />
</Canvas>

#### Usage

```js
import { DtInput, VALIDATION_MESSAGE_TYPES } from '@dialpad/handset';
```

```html
<dt-input
  v-model="text"
  placeholder="placeholder"
  :messages="[{
    message: 'This is an error message',
    type: VALIDATION_MESSAGE_TYPES.ERROR,
  }]"
/>
```

### Success

Notifies the user that the value(s) they've entered is correct.

<Canvas>
  <Story id="forms-input--with-success" />
</Canvas>

#### Usage

```js
import { DtInput, VALIDATION_MESSAGE_TYPES } from '@dialpad/handset';
```

```html
<dt-input
  v-model="text"
  placeholder="placeholder"
  :messages="[{
    message: 'This is a success message',
    type: VALIDATION_MESSAGE_TYPES.SUCCESS,
  }]"
/>
```

## Sizes

|name           |prop value                     |size|Example                                |
|---            |---                            |--- |---                                    |
|**Extra Small**|INPUT_SIZE_TYPES.EXTRA_SMALL   |12px|<Story id="forms-input--extra-small" />|
|**Small**      |INPUT_SIZE_TYPES.SMALL         |14px|<Story id="forms-input--small" />      |
|**Default**    |-                              |16px|<Story id="forms-input--default" />    |
|**Large**      |INPUT_SIZE_TYPES.LARGE         |20px|<Story id="forms-input--large" />      |
|**Extra Large**|INPUT_SIZE_TYPES.EXTRA_LARGE   |24px|<Story id="forms-input--extra-large" />|

### Usage

```js
import { DtInput, INPUT_SIZE_TYPES } from '@dialpad/handset';
```

```html
<dt-input
  v-model="text"
  placeholder="placeholder"
  :size="INPUT_SIZE_TYPES.<SIZE>"
/>
```
