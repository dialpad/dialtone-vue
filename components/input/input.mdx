...
- `maxLength` maximum length allowed to enter.
- `lengthDescription` text describing the maximum length allowed.
- `lengthValidationMessage` message to show in the validation message.
- `warnLengthThreshold` length from which the validation message will be shown as a warning.
...
<dt-input
  ...
  warn-length-threshold="15"
  length-description="Max 25 characters."
  length-validation-message="lengthValidationMessage"
/>
```

`currentLength` and `lengthValidationMessage` should be provided in your component. If you don't pass `currentLength`,
the component will use a built-in length calculation.
