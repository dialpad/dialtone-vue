...
Skeleton loader are grey animated shapes that visually mimics the content. They use a pulsing wave effect motion to
indicate they are loading. Use 2px rounded corners apart from circles. For text, use height that matches multiple of
8px. For images, match the actual image size. Skeleton's default variant is `label`.
...
and `aria-busy="false"` to corresponding loaded content html element.
