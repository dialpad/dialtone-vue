import { Canvas, Story, Subtitle, ArgsTable } from '@storybook/addon-docs';
...
that it can receive focus. This should be used for lists that contain mixed content, for example a list
in which some items are clickable/focusable and some are static.

When `navigation-type` is set to "arrow-keys" the props `index` and `isHighlighted` and an injected
method `setHighlight` should also be provided. The list item is not focusable, but it can be highlighted
using keyboard navigation. When the list item receives a highlight either by hovering over it with a
