...
It will be updated dynamically to be shown if there are or not recently used emojis (`recentlyUsedEmojis` prop).
...
When the user search for an emoji, the first emoji in the list will be with hovered style,
...
In the skin tone selector the user can press `ENTER` to open the skin tone selector and select the desire skin tone,
it can navigate through with `LEFT` and `RIGHT` arrows and select with `ENTER`.
