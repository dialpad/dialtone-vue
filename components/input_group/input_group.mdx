...
<Canvas>
  <Story id="forms-input-group--default" />
</Canvas>

## Variants

<Canvas>
  <Story id="forms-input-group--variants" />
</Canvas>
...
import {
  HsInputMixin,
  HsGroupableInputMixin,
} from '@dialpad/dialtone-vue';

export default {
  name: 'MyInputElement',

  mixins: [HsInputMixin, HsGroupableInputMixin],
...
```html
<hs-input-group name="my-group-name">
  <!-- Slotted Input Elements -->
  <template #legend>
    With Slotted Legend
  </template>
...
```html
<script>
import { HsInputGroup } from '@dialpad/dialtone-vue';
