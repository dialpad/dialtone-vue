...
when clicking outside the content, so it is important to use `.sync` on the `open`
...
Due to the different contexts in which a popover can be used, focus management and keyboard shortcuts like `ESC` are
