...
for a modal dialog that takes over the whole screen, use a HSDialog component.
...
```jsx
import { HsPopover } from '@dialpad/dialtone-vue';
```
...
using an element like an `HsButton` that is already accessible. The user should also be able to close the popover
...
```jsx
<template #anchor="{ attrs }">
  <hs-button v-bind="attrs">I'm accessible now!</hs-button>
</template>
```
...
provided for you. You are encouraged to consult the ARIA documentation for the particualr role you are implementing to
