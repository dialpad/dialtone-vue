...
# Handset Yeoman Generator

## Usage

We can use the Handset [Yeoman](https://yeoman.io/) Generator to initialize a new component within the Handset project.

```bash
# From Handset root directory
npx yo handset
```
...
Update(s) to the generator can be applied within the following file `generator-handset/generators/app/index.js`.

### Creating / Updating Template(s)

The templates used by the generator are located in the following directory `generator-handset/generators/app/templates`.
...
variable it will have to be defined in the generator `generator-handset/generators/app/index.js`.
